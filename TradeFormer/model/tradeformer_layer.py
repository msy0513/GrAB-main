...
class MultiHeadAttentionLayerGritSparse(nn.Module):
    ...
    def forward(self, batch):
        ...
        attn_weight = pyg_softmax(
            attn_weight, batch.edge_index[1]
        )  # (num relative) x num_heads x 1
        attn_weight = self.dropout(attn_weight)

        # Combine with multi-heads
        msg = (
            V_h[batch.edge_index[0]] * attn_weight
        )  # (num relative) x num_heads x out_dim
        wV = torch.zeros_like(V_h).to(
            torch.float32
...
class GritTransformerLayer(nn.Module):
    ...
    def __init__(
        ...
        self.norm1_h = nn.BatchNorm1d(
            out_dim, track_running_stats=True, eps=1e-5, momentum=bn_momentum
        )
        self.norm1_e = nn.BatchNorm1d(
            out_dim, track_running_stats=True, eps=1e-5, momentum=bn_momentum
        )
    ...
    def forward(self, batch):
        ...
        h = (torch.stack([h, h * log_deg], dim=-1) * self.deg_coef).sum(
            dim=-1
        )  # degree scaler
        h = self.out_h(h)
