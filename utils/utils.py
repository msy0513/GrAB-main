...
def save_model(model, base_dir, base_name):
    ...
    return get_path(base_dir, base_name, '.pt')
...
def get_metrics(y_hat, y_test, print_metrics=True):
    ...
    acc = accuracy_score(y_test, y_hat)
    pr = precision_score(y_test, y_hat, average='weighted')
    sn = recall_score(y_test, y_hat, average='weighted')
    sp = recall_score(y_test, y_hat, average='weighted')
    mcc = 0
    auroc = 0
    # auroc = roc_auc_score(y_test, y_hat, multi_class="ovo", average='weighted')
